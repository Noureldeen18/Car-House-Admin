...
-- Includes: Users, Products, Orders, Bookings, Services
-- ============================================
...
CREATE TABLE IF NOT EXISTS workshop_bookings (
  ...
  service_type TEXT NOT NULL, -- Keep for backward compatibility or direct text entry
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  type TEXT, -- 'order_update', 'system', etc.
  link TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- ADMINS Policies (Corrected to avoid recursion)
...
CREATE POLICY "order_items_insert_own" ON order_items FOR INSERT WITH CHECK (order_id IN (SELECT id FROM orders WHERE user_id = auth.uid()));
...
-- Insert sample service types if empty
INSERT INTO service_types (name, description, estimated_duration, base_price, icon, position) 
SELECT 'Oil Change', 'Complete oil and filter change service', 30, 150.00, '🛢️', 1
